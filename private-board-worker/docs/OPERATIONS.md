...
관리자 계정으로 로그인한 뒤 `관리자 설정`에서 통합 이미지 서비스를 등록하고 활성화하면 개발일지·자유게시판 이미지와 모든 로그인 회원의 개인 이미지 메뉴 및 API가 함께 열립니다. 별도의 개인 이미지 기능 플래그는 사용하지 않습니다.

활성화 전에 Workers VPC의 `IMAGE_VAULT` 바인딩과 관리자 설정의 이미지 서비스 업로드 토큰을 준비해야 합니다. 연결이 준비되지 않은 상태에서는 이미지 업로드가 설정 오류 toast로 거절됩니다.

기능을 다시 비활성화하면 이미지 메뉴와 애플리케이션 API 접근은 닫히지만, 이전에 복사된 공개 캐시 URL은 계속 접근할 수 있습니다. 일반 사용자의 삭제는 D1 보관함·게시글 연결만 제거하며 이미지 서비스 원본은 삭제하지 않습니다.
...
공유 코드는 공개 식별자이며 비밀값이 아닙니다. 공유 테마는 원본 행을 직접 참조하므로 원소유자의 수정이 모든 사용자에게 반영됩니다.

원본 테마 삭제 시 D1 트리거가 현재 사용자들을 기본 테마로 전환하고 일회성 안내 상태를 기록합니다. 테마 관련 장애를 조사할 때는 `custom_themes`, `user_shared_themes`, `user_theme_preferences`와 `custom_themes_before_delete` 트리거를 함께 확인하세요. 트리거를 제거하거나 테마 테이블을 수동 삭제하면 자동 복구와 안내가 작동하지 않을 수 있습니다.
