...
별도의 개인 이미지 기능 플래그는 사용하지 않습니다. 통합 이미지 서비스가 VPC에 연결되고 활성화되면 개발일지·자유게시판 이미지와 개인 이미지 메뉴 및 API가 함께 열립니다. 서비스를 비활성화하면 새 업로드와 메뉴는 닫히지만 이미 공유된 `/i/` URL과 이미지 서비스 원본은 유지됩니다.
