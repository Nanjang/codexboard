...
describe('핵심 화면', () => {
  ...
  it('개발일지 편집기에 간결한 공개 여부와 커서 위치 이미지 도구를 표시한다', async () => {
    ...
    expect(html).toContain('data-editor-command="insertHorizontalRule"')
    expect(html).toContain('가로줄')
  ...
  it('티켓 생성 폼에는 멱등성 요청값과 이중 제출 방지 표시가 있다', async () => {
    ...
    expect(html).not.toContain('여기에 놓아 상태를 변경')
  ...
  it('날씨 그래프는 부유 툴팁 없이 오늘 기준 고정 정보 행을 표시한다', async () => {
    ...
    expect(html).toContain('data-weather-focus-badges-toggle')
    expect(html).toContain('data-weather-focus-badges-toggle="true" checked=""')
    expect(html).toContain('오늘 기온 강조')
    expect(html.indexOf('class="weather-badge-toggle"')).toBeLessThan(html.indexOf('class="weather-range-label"'))
    expect(html).toContain('hidden')
  ...
  it('개인 이미지 목록에 캐시 URL, 복사 버튼, 복사 이력 아이콘을 표시한다', async () => {
    ...
    expect(html).toContain('class="image-upload-summary">업로드 메뉴</summary>')
    expect(html).toContain('data-image-upload-cancel')
    ...
    expect(html).toContain('>업로드 메뉴</summary>')
